...
def print_selection():
    if (var1.get() == 1) & (var2.get() == 0):
        config(text='I love Python ')
    elif (var1.get() == 0) & (var2.get() == 1):
        config(text='I love C++')
    elif (var1.get() == 0) & (var2.get() == 0):
        config(text='I do not anything')
    else:
        config(text='I love both')

var1 = IntVar()
var2 = IntVar()
c1 = Checkbutton(right_frame, text='Chiffe',variable=var1, onvalue=1, offvalue=0, command=generate_password)
c1.pack()
c2 = Checkbutton(right_frame, text='Caractère spéciaux',variable=var2, onvalue=1, offvalue=0, command=generate_password)
c2.pack(pady=20)
